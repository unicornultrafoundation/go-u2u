...
func (db *DBProducerWithMetrics) OpenDB(name string) (u2udb.Store, error) {
	...
	dm.log = logger
	dm.diskReadMeter = metrics.GetOrRegisterMeter("u2u/chaindata/"+strings.ReplaceAll(name, "-", "_")+"/disk/read", nil)
	dm.diskWriteMeter = metrics.GetOrRegisterMeter("u2u/chaindata/"+strings.ReplaceAll(name, "-", "_")+"/disk/write", nil)
