...
* Get node1 address:
```sh
go run ../cmd/u2u attach --exec "ftm.accounts[0]" http://localhost:4001
```
...
```sh
go run ../cmd/u2u attach --exec "ftm.getBalance(ftm.accounts[0])" http://localhost:4000
go run ../cmd/u2u attach --exec "ftm.getBalance(ftm.accounts[0])" http://localhost:4001
```
 outputs:
```js
