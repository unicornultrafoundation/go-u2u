...
//bash -c "cd ../../u2u-sfc && git checkout 02dd0ec0051c9faa192050a4156cb770175c4977"
// -c "docker run --name go-u2u-sfc-compiler -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../u2u-sfc:/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4' && docker commit go-u2u-sfc-compiler go-u2u-sfc-compiler-image && docker rm go-u2u-sfc-compiler"
// -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../u2u-sfc:/src -w /src go-u2u-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; rm -f /src/build/contracts/*json; npm run build'"
...
// -c "docker run --rm -v $(pwd)/contract/solc:/src/build/contracts -v $(pwd)/../../u2u-sfc:/src -w /src go-u2u-sfc-compiler-image bash -c 'export NPM_CONFIG_PREFIX=~; sed -i s/runs:\\ 200,/runs:\\ 10000,/ /src/truffle-config.js; rm -f /src/build/contracts/*json; npm run build'"
// -c "cd ../../u2u-sfc && git checkout -- truffle-config.js; docker rmi go-u2u-sfc-compiler-image"
