...
func TestCreateGas(t *testing.T) {
	for i, tt := range createGasTests {
		...
		doCheck := func(testGas int) bool {
			...
			vmenv := NewEVM(vmctx, TxContext{}, statedb, nil, params.AllProtocolChanges, config)
			var startGas = uint64(testGas)
			ret, gas, err := vmenv.Call(AccountRef(common.Address{}), address, nil, startGas, big.NewInt(0))
			if err != nil {
				return false
			}
